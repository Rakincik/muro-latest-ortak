...
(async () => {
    ...
    const getCoursesFromDoc = (doc) => {
        ...
        const els = Array.from(doc.querySelectorAll('a, button, [onclick], div.btn, span.btn'));
            ...
            if (identifier && !seenIdentifiers.has(identifier)) {
                ...
                try {
                    ...
                    for (let depth = 0; depth < 5; depth++) {
                        if (!parent) break;
                        const header = parent.querySelector('h1, h2, h3, h4, h5, strong, .card-title, .title, td:first-child');
    ...
    while (hasNextPage) {
        ...
        for (let i = 0; i < currentCourses.length; i++) {
            ...
            try {
                iframe.src = link;
                await sleep(3500); // Yüklenme ve AJAX bekleme süresi
    ...
    a.href = url;
    a.download = `akm_guncel_kayitlar.json`;
