...
for rec in all_recs:
    ...
    if best_mc and best_score >= 5:
        ...
        course_counts[best_mc] = course_counts.get(best_mc, 0) + 1
    else:
        unmatched_records_count += 1

print(f"\nTotal unique recordings: {len(all_recs)}")
print(f"Matched records: {matched_records_count}")
print(f"Unmatched records: {unmatched_records_count}")
print(f"Number of Muro courses with at least 1 video matched: {len(course_counts)} / {len(muro_courses)}")
