...
def generate_mock_email(name, existing_emails):
    ...
    email = f"{base_email}@ens.muro.click"
    counter = 1
    while email in existing_emails:
        email = f"{base_email}{counter}@ens.muro.click"
        counter += 1
...
def main():
    parser = argparse.ArgumentParser(description="MURO ENS - Okinar JSON Student & Course Importer")
    ...
    parser.add_argument("--recordings-json", default="okinar dersler/turkceoabtdeyiz.okinar.com_recordings.json", help="Path to scraped recordings JSON file")
    ...
    print(f"{Colors.CYAN}{Colors.BOLD}      MURO ENS - OKINAR JSON DATA IMPORT TOOL{Colors.ENDC}")
    print(f"{Colors.CYAN}{Colors.BOLD}============================================================{Colors.ENDC}")
    ...
    # Whitelisted Groups (Empty by default for ENS to import all groups)
    ...
    try:
        ...
        if scraped_recordings:
            ...
            for item in flat_recordings:
                ...
                if not c_id:
                    # Automatically create the missing course since it has a valid new recording!
                    c_id = str(uuid.uuid4())
                    cur.execute(
                        'INSERT INTO "Courses" ("Id", "Title", "IsDeleted", "IsPublished", "CourseType", "Mode", "Order", "CreatedAt") '
                        'VALUES (%s, %s, %s, %s, %s, %s, %s, %s)',
                ...
                video_url = f"https://canli.ens.muro.click/playback/presentation/2.3/{record_id}"
        ...
        if args.execute:
            ...
            print(f"    python import_ens_json.py --execute")
