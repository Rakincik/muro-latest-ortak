...
# 2. Build blacklist of other domains
blacklist_record_ids = set()
other_json_files = [
    'okinar dersler/4tuzem.okinar.com_recordings.json',
    'okinar dersler/akademikmasa.okinar.com_recordings.json',
    'okinar dersler/ataniyorumhocam.okinar.com_recordings.json',
    'okinar dersler/dershaneonline.okinar.com_recordings.json',
    'okinar dersler/mevzuatadam.okinar.com_recordings.json',
    'okinar dersler/omurhoca.okinar.com_recordings.json'
]

for f_path in other_json_files:
    with open(f_path, 'r', encoding='utf-8') as f:
        data = json.load(f)
    for item in data:
        if isinstance(item, dict) and 'recordID' in item:
            blacklist_record_ids.add(item['recordID'].strip())

print(f"Total blacklisted record IDs (from other domains): {len(blacklist_record_ids)}")
    ...
    with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
        for line in f:
            ...
            if match:
                ...
                # BLACKLIST FILTER: Exclude other domains!
                if folder_id in blacklist_record_ids:
                    continue
...
print("Parsing s4_videolar_listesi.txt with blacklist filter...")
s4_recs = parse_txt_file('mng/s4_videolar_listesi.txt', 's4')
print("Parsing s7_videolar.txt with blacklist filter...")
s7_recs = parse_txt_file('mng/s7_videolar.txt', 's7')

print(f"Post-blacklist recordings physically on s4: {len(s4_recs)}")
print(f"Post-blacklist recordings physically on s7: {len(s7_recs)}")

all_recs = s4_recs + s7_recs
print(f"Total unique MNG/YKS recordings present: {len(all_recs)}")
...
print(f"\nMatched Muro courses under blacklist filter: {len(matched_courses)} / {len(muro_courses)}")
