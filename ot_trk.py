...
def setup_environment():
    ...
    target_server = input(f"\n{Colors.BOLD}Anahtarın kopyalanacağı HEDEF sunucu IP (TRK BBB sunucusu - Varsayılan 185.111.247.86): {Colors.ENDC}").strip() or "185.111.247.86"
...
def transfer_records():
    ...
    source_dir = input(f"\n{Colors.BOLD}Transfer edilecek kaynak dizin yolu (Örn: /var/bigbluebutton/published/presentation): {Colors.ENDC}").strip()
    if not os.path.exists(source_dir):
        safe_print(f"\n{Colors.FAIL}[❌] HATA: Kaynak dizin '{source_dir}' bulunamadı.{Colors.ENDC}")
        return

    active_ids_file = "active_ids.txt"
    ...
    safe_print(f"[+] Eşleşen ve aktarılacak video klasörü sayısı: {len(folders_to_transfer)}")

    if not folders_to_transfer:
        safe_print(f"\n{Colors.WARNING}[!] Aktarılacak hiçbir eşleşen klasör bulunamadı.{Colors.ENDC}")
        return

    target_server = input(f"\n{Colors.BOLD}Hedef sunucu IP/Hostname (TRK BBB sunucusu - Varsayılan 185.111.247.86): {Colors.ENDC}").strip() or "185.111.247.86"
    ssh_user = input(f"{Colors.BOLD}SSH Kullanıcı Adı (Örn: root): {Colors.ENDC}").strip()
    ssh_port = input(f"{Colors.BOLD}SSH Portu (Varsayılan: 22): {Colors.ENDC}").strip() or "22"
    target_dir = input(f"{Colors.BOLD}Kayıt dizini (Varsayılan: /var/bigbluebutton/published/presentation): {Colors.ENDC}").strip() or "/var/bigbluebutton/published/presentation"
    ...
    max_workers_input = input(f"{Colors.BOLD}Paralel Aktarım Sayısı (Varsayılan: 5): {Colors.ENDC}").strip()
...
def main():
    ...
    while True:
        ...
        safe_print("2. Kayıt Transferini Başlat")
        safe_print("3. Çıkış")
