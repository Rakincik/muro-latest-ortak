...
for i, d in enumerate(data):
    ...
    uid = d['uid']
    sql_sessions.append(f"SELECT gen_random_uuid(), '{course_id}'::uuid, '{title}', '{url}', '{uid}', false, NOW(), 4, {i+1}, true, false, NOW(), NOW() WHERE NOT EXISTS (SELECT 1 FROM \"Sessions\" WHERE \"BbbMeetingId\" = '{uid}')")

query = f"""docker exec -i muro_3u_postgres psql -U muro_user -d muro_demo -c "INSERT INTO \\"Sessions\\" (\\"Id\\", \\"CourseId\\", \\"Title\\", \\"VideoUrl\\", \\"BbbMeetingId\\", \\"IsDeleted\\", \\"CreatedAt\\", \\"Status\\", \\"Order\\", \\"RecordingEnabled\\", \\"IsFree\\", \\"ScheduledStart\\", \\"ScheduledEnd\\") {' UNION ALL '.join(sql_sessions)};" """
