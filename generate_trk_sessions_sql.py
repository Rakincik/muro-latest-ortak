...
def main():
    ...
    sql_path = 'import_trk_sessions.sql'

    print(f"[*] Reading TRK (Atanıyorum Hocam) recordings from '{json_path}'...")
    try:
    ...
    sql_lines = [
        "-- ==================================================",
        "-- MURO LMS - TRK (ataniyorumhocam.okinar.com) Sessions Import",
    ...
    for item in recs:
        ...
        # Using the new TRK BBB play URL format!
...
BEGIN
    ...
    WHERE (TRIM(LOWER("Title")) = TRIM(LOWER('{safe_class_name}')) 
           OR TRIM(LOWER("Title")) = TRIM(LOWER(REPLACE('{safe_class_name}', '  ', ' '))))
      AND "IsDeleted" = false 
    LIMIT 1;
