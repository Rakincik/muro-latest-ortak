...
# 2. Load allowed record IDs from dereceuzem ONLY (Derece Uzem / MNG only!)
...
print(f"Total allowed MNG record IDs (Dereceuzem only): {len(allowed_record_ids)}")
    ...
    with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
        for line in f:
            ...
            if match:
                ...
                # STRICT FILTER: Only allow if present in MNG JSON!
...
print(f"Dereceuzem recordings physically on s4: {len(s4_recs)}")
print(f"Dereceuzem recordings physically on s7: {len(s7_recs)}")
...
for mc, mc_id in muro_courses.items():
    muro_parsed[mc] = {
        'id': mc_id,
        'words': set(get_words(mc) if 'get_words' in globals() else mc.split())
    }
...
for mc, mc_data in muro_parsed.items():
    mc_data['words'] = set(get_words(mc))
...
print(f"\nMatched Muro courses under dereceuzem ONLY: {len(matched_courses)} / {len(muro_courses)}")
